...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="site-shell">
    ...
    <article className="article-page page-wrap">
      ...
      <div className="content-columns"><div className="content-card article-body">{article.body.map((section, index) => <section key={index}>{section.heading && <h2>{section.heading}</h2>}{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}<p className="author-note">{article.authorNote}</p><p className="author-credit">{article.authorCredit}</p></div><aside className="admin-card"><p className="eyebrow">লেখাটির পরিচিতি</p><dl><div><dt>প্রকাশনা</dt><dd>{article.publication}</dd></div><div><dt>বিভাগ</dt><dd>{article.section}</dd></div><div><dt>লেখক</dt><dd>{article.author}</dd></div><div><dt>প্রকাশের তারিখ</dt><dd>{article.publishedDateLabel}</dd></div><div><dt>সংরক্ষণ অবস্থা</dt><dd>পূর্ণ লেখা</dd></div></dl><p className="source-url">মূল প্রকাশনার URL</p><a className="source-url-link" href={article.originalUrl} target="_blank" rel="noreferrer">{article.originalUrl}</a><p className="source-url">ছবির উৎস</p><a className="source-url-link" href={article.imageSourceUrl} target="_blank" rel="noreferrer">{article.imageCredit}</a></aside></div>
    </article>
